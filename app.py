...
# En app.py, reemplaza la función cargar_partida por esta versión final:

def cargar_partida(email):
    """
    Carga el estado de una partida, manejando comas decimales y datos inválidos.
    """
    try:
        ...
        if cell:
            ...
            if len(row_values) >= 4:
                # CORRECCIÓN CLAVE: Reemplazamos la coma por un punto
                saldo_str = row_values[1].replace(',', '.')
                ...
                game_over = bool(int(game_over_str)) if game_over_str and game_over_str.isdigit() else False
        ...
        st.error(f"Se encontró una partida guardada pero no se pudo procesar. Se creará una nueva.")
        # st.exception(e) # Opcional: descomentar para ver el error técnico en pantalla

    # Si la celda se encontró pero la fila era inválida, o si hubo otro error, se crea una nueva partida.
    return None
